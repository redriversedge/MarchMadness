...
- **ESPN API**: Public scoreboard endpoint (`groups=100` for tournament games). Team matching uses a comprehensive NAME_MAP in api.js with abbreviations, display names, and partial matches. Unmatched teams log to console. Adaptive polling: 2 min during live games, 2 min when games start within 30 min, 10 min idle.
...
- **Scores page**: Fourth tab showing all games organized by round with live scores (with period detail), final scores, or start date/time for scheduled games. Includes First Four play-in games (only final games shown, filtering out women's tournament). Shows team owners with color-coded badges. Has a Refresh button to manually trigger ESPN fetch. Auto-refreshes when ESPN polls complete. Games within each round sort: live first, then scheduled (by start time), then final at the bottom. Completed rounds sink below active/upcoming rounds. First Four section moves to the bottom when all play-in games are final. Player filter pills let you view only one player's games (e.g. Tom's teams).
